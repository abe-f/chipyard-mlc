...
object ReplacementPolicy {
  def fromString(s: String, n_ways: Int): ReplacementPolicy = s.toLowerCase match {
    ...
    case "ccplru" => new CCPseudoLRU(n_ways)
...
class CCPseudoLRU(n_ways: Int) extends ReplacementPolicy {
  // composable cachelets modified PLRU: http://www.cs.binghamton.edu/~dima/Sec22_CC.pdf
